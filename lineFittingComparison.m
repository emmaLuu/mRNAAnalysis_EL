...
for currentWindow = 1:windows
    ...
    if ~isempty(ampIntegralGauss3D)
        ...
        p2 = errorbar(traceFigTimeAxis(Particles{currentChannel}(currentParticle).FrameApproved),...
            ampIntegral3(Particles{currentChannel}(currentParticle).FrameApproved),...
            ones(length(ampIntegral3(Particles{currentChannel}(currentParticle).FrameApproved)),1)'*errorIntegral3,...
            '.-','Color','green','DisplayName','AmpIntegral3');
    end
    hold('off')
end
...
maxNumChanges = 3;
indexPoints = findchangepts(amplitudeToFit,...
    'Statistic','linear','MaxNumChanges',maxNumChanges);
coefficientsMethod3 = [];
...
%% testing cusum
figure('name',[prefix ' : particle ' num2str(currentParticle)])
subplot(1,2,1)
